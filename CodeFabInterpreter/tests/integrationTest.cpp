...
// IT_23: 중첩 인라인 if — 파일 모드
TEST_F(IntegrationFixture, IT_23_FileMode_NestedInlineIf)
{
    EXPECT_EQ(runFile({"if (true) if (true) if (true) print \"x\";"}), "x\n");
}

// IT_24: else 블록이 다음 줄에 오는 멀티라인 — 파일 모드
TEST_F(IntegrationFixture, IT_24_FileMode_ElseBlockNextLine)
{
    // 파서 검증 1: 단일 줄 if-else 블록
    EXPECT_EQ(run("if (true) { print \"a\"; } else { print \"b\"; }"), "a\n")
        << "single-line if-else block failed";
    // 파서 검증 2: 멀티라인 if-else 블록
    EXPECT_EQ(run("if (true)\n{\n  print \"a\";\n}\nelse\n{\n  print \"b\";\n}"), "a\n")
