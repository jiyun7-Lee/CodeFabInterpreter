...
{
    EXPECT_EQ(captureOutput([&]{ shell.runLine("print 1+2;"); }), "3\n");
}
...
TEST_F(ShellTest, VarDeclaration)
{
    ...
    EXPECT_EQ(captureOutput([&]{ shell.runLine("var y = 10;"); }), "");
}

// TC-PS-03: runLine 호출 간 변수 상태 유지
TEST_F(ShellTest, StatePersistence)
{
    captureOutput([&]{ shell.runLine("var x = 42;"); });
    EXPECT_EQ(captureOutput([&]{ shell.runLine("print x;"); }), "42\n");
}
...
TEST_F(ShellTest, CheckerError)
{
    ...
    EXPECT_EQ(captureOutput([&]{ shell.runLine("print 1;"); }), "1\n");
}
...
TEST_F(ShellTest, RuntimeError)
{
    ...
    EXPECT_EQ(captureOutput([&]{ shell.runLine("print 2;"); }), "2\n");
}
...
TEST_F(ShellTest, IfStatementInShell)
{
    ...
    EXPECT_EQ(captureOutput([&]{ shell.runLine("if (x > 0) { print x; }"); }), "5\n");
}
...
TEST_F(ShellTest, IncompleteIfShellContinues)
{
    ...
    EXPECT_EQ(captureOutput([&]{ shell.runLine("print x;"); }), "5\n");
}
...
TEST_F(ShellTest, ExitQuitCommand)
{
    for (const auto& cmd : {"exit", "EXIT", "Exit", "quit", "QUIT", "Quit"})
        EXPECT_EQ(captureOutput([&]{ shell.runLine(cmd); }), "") << "input: " << cmd;
}
...
TEST_F(ShellTest, CheckerErrorDoesNotCorruptState)
{
    ...
    EXPECT_EQ(captureOutput([&]{ shell.runLine("print a;"); }), "5\n");
}

// TC-PS-11: 별도 runLine 호출에서 동일 변수명을 재선언하면 Checker 에러
TEST_F(ShellTest, DuplicateVarAcrossRunLine)
{
    shell.runLine("var x = 1;");

    auto errOut = captureOutput([&]{ shell.runLine("var x = 2;"); });
    EXPECT_NE(errOut.find("[Checker]"), std::string::npos);

    EXPECT_EQ(captureOutput([&]{ shell.runLine("print x;"); }), "1\n");
}

// TC-PS-12: 파서 에러 발생 시 [Error] prefix가 출력된다
TEST_F(ShellTest, ParserErrorOutputsErrorPrefix)
{
    auto out = captureOutput([&]{ shell.runLine("if (true)"); });
...
TEST_F(ShellTest, FunctionPersistsAcrossRunLine)
{
    ...
    EXPECT_EQ(captureOutput([&]{ shell.runLine("print add(3, 4);"); }), "7\n");
}
