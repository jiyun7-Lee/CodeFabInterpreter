...
{
    EXPECT_EQ(captureOutput([&]{ runner.runSource({"var a = 10;", "print a;"}); }), "10\n");
}

// TC-FILE-02: 존재하지 않는 파일 경로 → "File Not Found" 출력
TEST_F(FileModeTest, TC_FILE_02_FileNotFound)
{
    auto out = captureOutput([&]{ runner.run("__nonexistent_file__.cf"); });
...
{
    auto out = captureOutput([&]{ runner.runSource({"var a = 10;", "print 1/0;"}); });
...
{
    auto out = captureOutput([&]{ runner.runSource({"var a = 5;", "var b = b;"}); });
...
{
    auto out = captureOutput([&]{ runner.runSource({"print 1/0;", "print 99;"}); });
