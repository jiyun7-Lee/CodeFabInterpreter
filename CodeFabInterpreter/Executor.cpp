...
Value Executor::visitLiteral(LiteralExpr& e)
{
    return e.value;
}

Value Executor::visitVariable(VariableExpr& e)
{
    return e.distance >= 0 ? env_->getAt(e.distance, e.name.lexeme)
                           : env_->get(e.name.lexeme, e.name.line);
}

Value Executor::visitGrouping(GroupingExpr& e)
{
    return evaluateExpr(e.expression.get(), env_);
}

Value Executor::visitUnary(UnaryExpr& e)
{
    Value val = evaluateExpr(e.right.get(), env_);
...
Value Executor::visitAssign(AssignExpr& e)
{
    Value val = evaluateExpr(e.value.get(), env_);
    if (e.distance >= 0)
        env_->assignAt(e.distance, e.name.lexeme, val);
    else
...
Value Executor::visitBinary(BinaryExpr& e)
{
    ...
    if (e.op.type == TokenType::AND)
    {
        ...
        return evaluateExpr(e.right.get(), env_);
    }
...
Value Executor::visitFunctionCall(FunctionCallExpr& e)
{
    ...
    if (e.callee.lexeme == "Array" && functions_.find("Array") == functions_.end())
    {
        ...
        Value sizeVal = evaluateExpr(e.args[0].get(), env_);
    ...
    for (size_t i = 0; i < fn.params.size(); ++i)
        fnEnv.define(fn.params[i].lexeme, evaluateExpr(e.args[i].get(), env_));
...
Value Executor::visitArrayLiteral(ArrayLiteralExpr& e)
{
    auto arr = std::make_shared<ArrayValue>();
    for (const auto& elem : e.elements)
        arr->elements.push_back(evaluateExpr(elem.get(), env_));
    return arr;
}

Value Executor::visitArrayAccess(ArrayAccessExpr& e)
{
    Value arrVal = evaluateExpr(e.array.get(), env_);
    ...
    Value idxVal = evaluateExpr(e.index.get(), env_);
...
Value Executor::visitArrayWrite(ArrayWriteExpr& e)
{
    Value arrVal = evaluateExpr(e.array.get(), env_);
    if (!std::holds_alternative<std::shared_ptr<ArrayValue>>(arrVal))
        throw std::runtime_error("Type error: not an array");
    ...
    if (idx < 0 || idx >= static_cast<int>(arr->elements.size()))
        throw std::runtime_error("Array index out of bounds");
    Value newVal = evaluateExpr(e.value.get(), env_);
