...
> `<=` `>=` `==` `!=` 는 프로젝트 스펙에 포함되지 않아 현재 미구현입니다.

`and` / `or` 는 단락 평가(Short-circuit)를 수행합니다. 결과가 확정된 시점에 우변 평가를 건너뜁니다.
...
에러 발생 시 단계를 나타내는 접두사와 함께 메시지를 출력합니다.  
파일 / 디버그 모드에서는 에러 발생 시 해당 소스 실행을 즉시 중단합니다.

| 단계 | 출력 접두사 | 주요 케이스 |
|------|------------|------------|
| Parser | `[Error]` | 세미콜론 누락, 닫는 괄호 누락, 잘못된 할당 대상, 예상치 못한 토큰 |
| Checker | `[Checker]` | 자기 초기화(`var a = a`), 같은 스코프 중복 선언 |
| Runtime | `[Error]` | 미정의 변수, `+` 타입 불일치, 단항 `-` 타입 오류 |

네거티브 케이스별 상세 명세 및 테스트: [TC_NegativeTest.md](CodeFabInterpreter/TC_NegativeTest.md)
...
| 말머리 | 용도 |
|--------|------|
| `**Positive**` | PR 전체 리뷰 본문에서 잘된 점 언급 |
| `**[Minor]**` | 사소한 개선 제안 — 블로커 아님, 합의 후 반영 여부 결정 |
| (없음) | 반드시 수정 필요한 블로커 — 태그 없이 명확하게 기술 |
